...
public class VoiceKSDHActiivty extends BaseChatActivity{
    ...
    @BindView(R.id.voice_touser_personimage)
    ImageView voiceTouserPersonimage;
    @BindView(R.id.voice_touser_name)
    TextView voiceTouserName;
    @BindView(R.id.voice_touser_type)
    TextView voiceTouserType;
    @BindView(R.id.voice_time)
    TextView voiceTime;
    @BindView(R.id.voice_touser_line)
    LinearLayout voiceTouserLine;
    @BindView(R.id.voice_checkvoice)
    CheckBox voiceCheckvoice;
    @BindView(R.id.voicejieshou)
    ...
    @OnClick({R.id.voice_checkvoice, R.id.voicejieshou, R.id.voice_cancel})
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.voice_checkvoice:
                setSpeakerphone();
                break;
            case R.id.voicejieshou:
                stopMusic();
    ...
    private void getdata() {
        if (callInfor == null) {
            Toast.makeText(VoiceKSDHActiivty.this, "信息不完整", Toast.LENGTH_SHORT).show();
            return;
        }
        if (call_type.equals("go")) {
            if (callInfor.getFrom().equals("") || callInfor.getTo().equals("") || callInfor.getType().equals("")) {
                Toast.makeText(VoiceKSDHActiivty.this, "拨打信息不完整", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            getCallInfor(new NetWorkResult() {
                ...
                public void result(int code, String data) {
                    if (VoiceKSDHActiivty.this != null && !VoiceKSDHActiivty.this.isFinishing()) {
                        if (code == 0) {
